...
- [ ] **Happy path тесты** — основной сценарий покрыт
- [ ] **Edge cases** — граничные случаи (пустые данные, null, максимумы)
- [ ] **Error handling тесты** — проверка обработки ошибок
- [ ] **Тесты УПАЛИ (RED state)** — если TDD, тесты должны падать до реализации
- [ ] **Нет `.only()` или `skip`** — все тесты активны
- [ ] **Descriptive names** — названия тестов понятны
...
### Статус: RED (готово для code-agent) / ТРЕБУЕТСЯ УТОЧНЕНИЕ
```
