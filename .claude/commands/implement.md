...
**ОБЯЗАТЕЛЬНО** используй Task tool для делегирования агентам.
Это НЕ опционально — агенты ДОЛЖНЫ использоваться.

### Фаза 1: Тесты (test-agent)

**ОБЯЗАТЕЛЬНО** делегируй test-agent через Task:

```
[Task: test-agent]
prompt: |
  ## Задача: Написать тесты по плану
  ...
  2. Напиши тесты для каждого acceptance criteria
  3. Тесты ДОЛЖНЫ УПАСТЬ (RED state)
  4. Не пиши код реализации — только тесты

  ### Выход
  - Файлы тестов созданы
  - Тесты запущены и УПАЛИ (это правильно)
  - Список тестов и что они проверяют
```

Дождись завершения test-agent.

### Фаза 2: Код (code-agent)

**ОБЯЗАТЕЛЬНО** делегируй code-agent через Task:
...
prompt: |
  ...
  ### Контекст
  - План: .claude-workspace/current-task.md
  - Тесты: уже написаны test-agent'ом
  - Прогресс: .claude-workspace/progress.md

  ### Инструкции
  1. Прочитай план из current-task.md
  2. Реализуй минимум кода для прохождения тестов
  3. Атомарные коммиты после каждого шага
  4. Обновляй progress.md

  ### Ограничения
  - Тесты уже есть — не переписывай их
  - Один шаг = один коммит
  - Код всегда в рабочем состоянии
```

Дождись завершения code-agent.
...
После завершения покажи структурированный вывод:
...
### Следующие шаги
- [ ] Запусти /code-review для код-ревью
```