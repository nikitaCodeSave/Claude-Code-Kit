...
**РЕКОМЕНДУЕТСЯ** использовать Task tool для делегирования review-agent:

```
[Task: review-agent]
prompt: |
  ## Задача: Код-ревью недавних изменений

  ### Контекст
  - План: .claude-workspace/current-task.md
  - Scope: $ARGUMENTS (или auto-detect)

  ### Инструкции
  1. Определи scope ревью (staged/branch/N коммитов)
  2. Проверь соответствие плану
  3. Запусти автоматические проверки (tests, lint)
  4. Проверь безопасность
  5. Выдай вердикт: APPROVED / CHANGES REQUESTED / REJECTED
