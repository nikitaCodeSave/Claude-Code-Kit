...
4. Используй `/create-plan` для фич > 50 строк кода
5. Используй `/code-review` после реализации

## Команды

| Команда | Когда использовать |
|---------|-------------------|
| `/create-plan [feature]` | Перед реализацией фич > 50 строк кода |
| `/implement` | После одобрения плана, выполняет TDD workflow |
| `/code-review` | После реализации, перед merge |
| `/complete-task` | После APPROVED ревью, архивирует задачу |
...
```
/create-plan → current-task.md → /implement → атомарные коммиты → /code-review → /complete-task → merge
```
